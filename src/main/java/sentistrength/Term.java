...
public class Term {
   ...
   private boolean bgIsInterruptJudged=false;
   private boolean igInterrupt;
   public boolean isInterrupt() {
	   if(!bgIsInterruptJudged) {
		   this.igInterrupt=judgeInterrupt();
		   bgIsInterruptJudged=true;
	   }
	   return this.igInterrupt;
	}
   ...
   public String getSgPunctuation() {
	   return sgPunctuationEmphasis;
   }

   public void setSgPunctuation(String sgPunctuation) {
	   this.sgPunctuation = sgPunctuation;
   }
   ...
   public int extractNextWordOrPunctuationOrEmoticon(String sWordAndPunctuation, ClassificationResources classResources, ClassificationOptions classOptions) {
      ...
      else if( sWordAndPunctuation.equals("+1") ) {
   ...
   public int getSentimentValue() {
      if (this.bgOverrideSentimentScore) {
         return this.igOverrideSentimentScore;
    	  ...
    	  int senti=this.getSentimentID() < 1 ? 0 : this.resources.sentimentWords.getSentiment(this.igWordSentimentID);
          return senti;
      }
   }
   ...
   public void setBoosterWordScore(int num) {
	      this.igBoosterWordScore=num;
   }
   ...
   public boolean judgeInterrupt() {
	   String word=getOriginalText();
	   String[] possWrd= {"?","!",".",":"};
   ...
   //关于否定词缀形成反义词的探索；
   private static HashMap<String,Integer> antDict = new HashMap<String,Integer>();
   public int getAntWordSenti(ArrayList<String> ants) {
	   if(antDict.containsKey(getText())) {
		   return antDict.get(getText());
	   }
	   for(int i=0;i<ants.size();i++) {
		   int antSenti = getSentimentValue( ants.get(i) );
		   if(antSenti!=0) {
			   antDict.put(getText(),antSenti);
			   return antSenti;
		   }
	   }
	   return 0;
   ...
   public int getSentimentValue(String lemma) {
	   int sentiID = this.resources.sentimentWords.getSentimentID(lemma);
	   int senti = sentiID < 1 ? 0 : this.resources.sentimentWords.getSentiment(sentiID);
       return senti;
   }
}
